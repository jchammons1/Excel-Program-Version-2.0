VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} UserForm_BidDatePicker 
   Caption         =   "BID DATE SELECTION"
   ClientHeight    =   5025
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   6660
   OleObjectBlob   =   "UserForm_BidDatePicker.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "UserForm_BidDatePicker"
...
'Send the data to the Project_Data Sheet for future use in other sheets.  It stores everytime the project data is saved.
'The project data for the BIDTAB Estimate will be saved to the third row (row 4) always
    With Me
        PROJECT_DATA.Visible = True
        PROJECT_DATA.Select
        Range("AH2").Value = Me.TextBox1.Value
            Selection.NumberFormat = "[$-en-US]m/d/yyyy"
        Range("AH3").Value = Me.TextBox1.Value
            Selection.NumberFormat = "[$-en-US]m/d/yyyy"
        Range("AH4").Value = Me.TextBox1.Value
            Selection.NumberFormat = "[$-en-US]m/d/yyyy"
        Range("AH5").Value = Me.TextBox1.Value
            Selection.NumberFormat = "[$-en-US]m/d/yyyy"
...
Private Sub UserForm_Initialize()
    buildCalendar
    TextBox1.Value = Range("AH2").Value
End Sub
